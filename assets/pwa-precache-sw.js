...
},{"charenc":3,"crypt":327,"is-buffer":328}],330:[function(require,module,exports){
'use strict';

/**
 * Source:
 * https://github.com/InvokIT/js-untar,
 * but without the extra Worker (run in SW).
 */
/* eslint-disable */
...
function untar(buf) {
    ...
    var tarFileStream = new UntarFileStream(buf);
    while (tarFileStream.hasNext()) {
        var file = tarFileStream.next();
        files.push(decorateExtractedFile(file));
    }
...
var decoratedFileProps = {
    ...
    readAsString: {
        value: function value() {
            ...
            return this._string = charCodes.map(function (cc) {
...
UntarStream.prototype = {
    readString: function readString(charCount) {
        var charSize = 1;
        var byteCount = charCount * charSize;
    ...
    readBuffer: function readBuffer(byteCount) {
        var buf = void 0;
    ...
    seek: function seek(byteCount) {
        this._position += byteCount;
    },

    peekUint32: function peekUint32() {
        return this._bufferView.getUint32(this.position(), true);
    },

    position: function position(newpos) {
        if (newpos === undefined) {
 ...
 * @author Aapo Kiiso <aapo@lamia.fi>
 *
 */
...
var md5 = require('md5');
var untar = require('./includes/untar');

/**
 * @param {String} apiUrl - pre-caching API root URL
 * @param {String} clientId - pre-caching client identifier
 * @param {String} archiveName - current archive used by pre-caching client
 * @param {String} [fallbackUrl] - fallback document url if no pre-cache or network
 */
self.preCacheSW = function (apiUrl, clientId, archiveName) {
    var initPreCache = function () {
        var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
            var _ref2, _ref3, cache, archive, cacheResponses;
            ...
            return regeneratorRuntime.wrap(function _callee$(_context) {
                while (1) {
                    switch (_context.prev = _context.next) {
                        case 0:
                            _context.next = 2;
                            return Promise.all([getCleanPreCache(), getResponseArchive()]);
                        ...
                        case 2:
                            ...
                            cache = _ref3[0];
                            archive = _ref3[1];
                            cacheResponses = archive.map(function (archiveEntry) {
                                var url = archiveEntry.name,
                                    status = archiveEntry.status,
                                    headers = archiveEntry.headers,
                                    content = archiveEntry.blob;


                                if (!archiveEntry.status) {
                                    // Status is required from a response.
                                    // If no status, this is probably the file index -> skip.
                                    return null;
                                }

                                try {
                                    var response = new Response(content, { url: url, status: status, headers: headers });
        ...
        return function initPreCache() {
            return _ref.apply(this, arguments);
        };
    }();

    var getCleanPreCache = function () {
                    ...
                    switch (_context2.prev = _context2.next) {
                        case 0:
                            ...
                            return caches.keys().then(function (cacheNames) {
                                var removeOldPreCaches = cacheNames.filter(function (cacheName) {
                                    return (/precache-[a-z0-9]/.test(cacheName)
                                    );
                                }).map(function (cacheName) {
                                    return caches.delete(cacheName);
                                });

                                return Promise.all(removeOldPreCaches);
                            });

                        case 2:
                            return _context2.abrupt('return', getPreCache());
        ...
        return function getCleanPreCache() {
            return _ref4.apply(this, arguments);
        };
    }();

    var fetchFromPreCache = function () {
        var _ref5 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3(request) {
            var cache, responseFromCache;
                ...
                while (1) {
                    switch (_context3.prev = _context3.next) {
                        ...
                        case 2:
                            ...
                            _context3.t0 = canFetchFromPreCache(request);
                        ...
                        case 8:
                            responseFromCache = _context3.t0;
                            ...
                            // ... but also refresh resource in cache in the background if found.
                            fetch(request).then(function (response) {
                                return cache.put(request, response);
                            }).catch(function () {
                                // Ignore errors, refresh is just "nice-to-have".
                            });

                            return _context3.abrupt('return', responseFromCache);

                        case 12:
                            return _context3.abrupt('return', fetchFromNetwork(request, false));
        ...
        return function fetchFromPreCache(_x2) {
            return _ref5.apply(this, arguments);
        };
    }();

    var fetchFromNetwork = function () {
        var _ref6 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee4(request) {
            var tryPrecacheIfFail = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
            var responseFromNetwork, responseFromCache, fallbackDocResponse;
            return regeneratorRuntime.wrap(function _callee4$(_context4) {
                while (1) {
                    switch (_context4.prev = _context4.next) {
                        ...
                        case 3:
                            responseFromNetwork = _context4.sent;
                            ...
                            return _context4.abrupt('return', responseFromNetwork);
                        ...
                        case 8:
                            ...
                            if (!tryPrecacheIfFail) {
                                _context4.next = 16;
                                break;
                            }

                            _context4.next = 13;
                            return fetchFromPreCache(request);

                        case 13:
                            responseFromCache = _context4.sent;
                            ...
                            return _context4.abrupt('return', responseFromCache);

                        case 16:
                            if (!(tryPrecacheIfFail && shouldFetchFallbackDoc(request))) {
                                _context4.next = 22;
                                break;
                            }

                            _context4.next = 19;
                            return fetchFromPreCache(fallbackUrl);

                        case 19:
                            fallbackDocResponse = _context4.sent;

                            if (!fallbackDocResponse) {
                                _context4.next = 22;
                                break;
                            }

                            return _context4.abrupt('return', fallbackDocResponse);

                        case 22:

                            // Log network error as ultimate fallback.
        ...
        return function fetchFromNetwork(_x4) {
    ...
    var getResponseArchive = function () {
        var _ref7 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee5() {
            var res, buf, archive, fileIndex;
            return regeneratorRuntime.wrap(function _callee5$(_context5) {
                while (1) {
                    switch (_context5.prev = _context5.next) {
                        case 0:
                            _context5.next = 2;
                            return fetch(getArchiveDownloadUrl());
                        ...
                        case 8:
                            ...
                            // Add response metadata to archive entries.

                            fileIndex = archive.find(function (file) {
                                return file.name === 'index.json';
                            }).readAsJSON();


                            archive.forEach(function (archiveEntry) {
                                var indexEntry = fileIndex.find(function (indexEntry) {
                                    return md5(indexEntry.url) === archiveEntry.name;
                                });

                                if (indexEntry) {
                                    var name = indexEntry.url,
                                        status = indexEntry.status,
                                        headers = indexEntry.headers,
                                        mimeType = indexEntry.mimeType;

                                    archiveEntry.name = name;
                                    archiveEntry.status = status;
                                    archiveEntry.headers = headers;
                                    archiveEntry.mimeType = mimeType;
                                }
                            });

                            return _context5.abrupt('return', archive);
        ...
        return function getResponseArchive() {
            return _ref7.apply(this, arguments);
        };
    }();

    var fallbackUrl = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';

    var preCacheName = 'precache-' + archiveName;

    self.addEventListener('install', function (event) {
        event.waitUntil(initPreCache().then(function () {
    ...
    self.addEventListener('fetch', function (event) {
        ...
        var resolveFetch = function resolveFetch() {
            if (!isNetworkPreferred(request)) {
                return fetchFromPreCache(request);
            }
        ...
        event.respondWith(resolveFetch());
    });
    ...
    function getArchiveDownloadUrl() {
        return apiUrl + '/client/' + clientId + '/archive';
    }

    /**
     * Serve some responses from network instead
     * of pre-cache if possible. Applies to dynamic
     * content (documents) and images, which are compressed
     * in pre-cache and look bad. Progressive image solution
     * should be built to fix that.
     *
     * @param {Request} request
     * @returns {boolean}
     */
    function isNetworkPreferred(request) {
        if (!canFetchFromPreCache(request)) {
            return true;
        }

        var networkMimes = ['image/*', 'text/html'];
        var acceptMimes = getAcceptedMimeTypes(request);

        return acceptMimes.filter(function (mime) {
            return networkMimes.includes(mime);
        }).length;
    }

    /**
     * Fallback doc is served for the first request
     * arriving from the same host as the fallback doc
     * with the correct type.
     *
     * @param {Request} request
     * @returns {boolean}
     */
    function shouldFetchFallbackDoc(request) {
        var urlParts = new URL(request.url);
        var fallbackUrlParts = new URL(fallbackUrl);
        var matchingOrigin = urlParts.origin === fallbackUrlParts.origin;

        var acceptMimes = getAcceptedMimeTypes(request);

        return matchingOrigin && acceptMimes.includes('text/html');
    }

    function getAcceptedMimeTypes(request) {
        var acceptHeader = request.headers.get('Accept');

        return acceptHeader ? acceptHeader.split(',') : [];
    }

    function canFetchFromPreCache(request) {
        var precacheOptions = request.headers && request.headers.get('X-Precache-Options');

        return precacheOptions !== 'deny';
    }
};

},{"./includes/untar":330,"babel-polyfill":1,"md5":329}]},{},[331]);
